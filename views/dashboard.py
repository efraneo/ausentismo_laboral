...
def render_dashboard():
    ...
    with col_a: 
        st.plotly_chart(chart_dias_por_mes(df_aus), use_container_width=True, key="grafico_dias_mes")
    with col_b: 
        st.plotly_chart(chart_ausentismo_por_tipo(df_aus), use_container_width=True, key="grafico_tipo_aus")

    st.plotly_chart(chart_ausentismo_por_proceso(df_aus), use_container_width=True, key="grafico_proceso")

    col_c, col_d = st.columns(2)
    with col_c: 
        st.plotly_chart(chart_top_diagnosticos(df_aus), use_container_width=True, key="grafico_diag")
    with col_d: 
        st.plotly_chart(chart_accidentes_por_dia_semana(df_aus), use_container_width=True, key="grafico_dias_sem")

    st.markdown("---")
    st.markdown("### 📋 Permisos Laborales")
    col_e, col_f = st.columns(2)
    with col_e: 
        st.plotly_chart(chart_permisos_por_mes(df_per), use_container_width=True, key="grafico_perm_mes")
    with col_f: 
        st.plotly_chart(chart_permisos_por_tipo(df_per), use_container_width=True, key="grafico_perm_tipo")
